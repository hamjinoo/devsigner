...
function normalize(entry: SeedEntry): NormalizedReference {
  return {
    ...
    typography: {
      ...
      headingFont: entry.typography.heading_font ?? null,
      bodyFont: entry.typography.body_font ?? null,
    ...
    spacing: {
      ...
      gridAlignedPct: entry.spacing?.grid_aligned_pct ?? 0,
      density: (entry.spacing?.density ?? "balanced") as "spacious" | "balanced" | "compact",
      uniqueCount: entry.spacing?.unique_count ?? 0,
    },
    layout: entry.layout ?? { maxWidth: null, hasSidebar: false, hasHero: false, hasStickyHeader: false, hasFooter: false },
    shapes: {
      cornerStyle: (entry.shapes?.corner_style ?? "subtle") as NormalizedReference["shapes"]["cornerStyle"],
      shadowStyle: (entry.shapes?.shadow_style ?? "none") as NormalizedReference["shapes"]["shadowStyle"],
